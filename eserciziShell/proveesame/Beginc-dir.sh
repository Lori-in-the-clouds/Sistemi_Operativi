...
#controllo che f sia una directory e che sia traversabile 
if test ! -d $f -o ! -x $f
then echo Errore: $f non directory o non attraversabile: exit 6
fi
PATH=`pwd`:$PATH
export PATH
echo Beginc.sh: Stiamo per esplorare la directory $d
Cercadir.sh $d $f	#invocazione del secondo file comandi (che e' quello RICORSIVO e fa in questo caso tutto il 'lavoro'!)
