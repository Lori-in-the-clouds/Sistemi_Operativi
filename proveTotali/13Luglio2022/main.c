...
int main(int argc,char **argv) {
    ...
    int L; /*per la lunghezza in linea del file*/
    ...
    int occ; /*nuemro di occorrenze del carattere Cq nella riga selezionata*/
    /*----------------------------------------------*/
    ...
    for(q=0;q<Q;q++) {
        ...
        if(pid == 0) {
           ...
           for(j=0;j<Q+1;j++) {
            if(j != q)
                close(piped[j][0]);
            if(j != (q+1)) // IN QUESTO CASO VOGLIO CHE L'ULTIMO FIGLIO COMUNICHI CON IL PADRE E NON CON IL PRIMO FIGLIO
                close(piped[j][1]);
           }
           ...
           occ=0; /*azzeriamo la variabile occ*/
           while(read(fd,&ch,1)) {
                if (ch == argv[q+3][0]) occ++;

                if (ch=='\n') {
                    /*siamo arrivati a fine riga*/

                    /*aspettiamo l'ok da parte del figlio/padre*/
                    /*leggiamo dalla pipe*/
                    ...
                    /*mandiamo l'ok al figlio successivo*/
                    /*scriviamo sulla pipe*/
                    nw = write(piped[q+1][1],&ok,sizeof(ok));
                    if( nw != sizeof(ok)) {
                        printf("Errore: scrittura di un numero di byte sbagliati %d\n",nw);
                        exit(-1);
                    }
                    
                    /*azzeriamo il numero di occorrenze*/
                    ritorno = occ;
                    occ=0;
                }
           }
    ...
    for (j=0;j<Q+1;j++) {
       if (j != Q) close(piped[j][0]);
       if (j != 0) close(piped[j][1]);
    }
    ...
    for (i=1;i<=L;i++) {
        ...
        /*leggiamo l'ok inviato dall'ultimo figlio*/
        /*leggiamo dalla pipe*/
        nr = read(piped[Q][0],&ok,sizeof(ok));
        if( nr != sizeof(ok)) {
            printf("Errore: lettura di un numero di byte sbagliati %d\n",nr);
