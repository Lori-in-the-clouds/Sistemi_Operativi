...
{   
   ...
   int pidFiglio, status, ritorno;      /* per wait padre */
	...
	/* padre */
	close(piped[1]); /* chiudo scrittura */		
    ...
    int ris;
	while (read(piped[0],&ris,sizeof(int)))  /* questo ciclo avra' termine appena il figlio terminera' dato che la read senza piu' scrittore tornera' 0! */
	{ 
            read(piped[0],mess,ris);
        	/* dato che il figlio gli ha inviato delle stringhe, il padre le puo' scrivere direttamente con una printf */
        	printf("%d: %s --- Lunghezza della stringa: %d\n", j, mess,ris-1);
        	j++;
	}
